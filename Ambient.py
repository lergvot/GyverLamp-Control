...
SAT_BOOST = 1.2   # Усиление насыщенности (>1 - ярче цвета)
...
UPDATE_INTERVAL = 0.2    # Интервал обновления (секунды)
SAMPLE_SIZE = 100         # Размер уменьшенного изображения для анализа (30-50 макс производительность, 100-200 хорошая точность, 300+ почти полное качество)
COLOR_CHANGE_THRESHOLD = 10  # Порог изменения цвета для обновления лампы (5-15 - высокая чувствительность, 20-40 - средняя чувствительность, 50+ - низкая чувствительность)

# Настройки плавности переходов
SMOOTH_TRANSITIONS = True  # Включить плавные переходы
TRANSITION_STEPS = 5       # Количество шагов для плавного перехода
...
class LampSync:
    ...
    def print_monitor_info(self):
        ...
        for i, monitor in enumerate(self.monitors):
            if i == 0:
                ...
                print(f"  {i}: Монитор {i} - {monitor['width']}x{monitor['height']} "
    ...
    def select_monitor(self, index: int):
        ...
        if index == 0:
            print(f"Выбрана объединенная область всех мониторов: {monitor['width']}x{monitor['height']}")
        else:
    ...
    def smooth_transition(self, start_rgb: Tuple[int, int, int], 
                         target_rgb: Tuple[int, int, int]) -> list:
    ...
    def set_lamp_color(self, r: int, g: int, b: int):
        ...
        print(f"RGB({r}, {g}, {b}) -> Лампa: BRI:{brightness} SPD:{saturation} SCA:{hue}")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    color_diff = (abs(r - self.last_rgb[0]) + 
                    ...
                    frame_count += 1
                    time.sleep(0.01)  # Короткая задержка для снижения нагрузки на CPU
